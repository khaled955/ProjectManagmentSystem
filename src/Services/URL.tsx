...
export const AUTHENTICATION_URL = {
    ...
    GET_ALL_USERS:(pageSize:number,pageNumber:number,userName:string="",groups:number)=>`${baseURL}/api/v1/Users/?userName=${userName}&groups=${groups}&pageSize=${pageSize}&pageNumber=${pageNumber}`,
...
GET_PROJECTS_FOR_TASKS_FORM:(pageSize:number,pageNumber:number)=>`${baseURL}/api/v1/Project/?pageSize=${pageSize}&pageNumber=${pageNumber}`
...
export  const TASKS_URL={
  ...
  GET_ALL_ASSIGNED_TASKS:(pageSize:number,pageNumber:number,title:string="",status:string ="")=>`${baseURL}/api/v1/Task?title=${title}&status=${status}&pageSize=${pageSize}&pageNumber=${pageNumber}`,
